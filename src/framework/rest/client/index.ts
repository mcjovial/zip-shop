...
class Client {
  ...
  orders = {
    all: (params: Partial<OrderQueryOptions>) =>
      HttpClient.get<OrderPaginator>(API_ENDPOINTS.MY_ORDERS, {
        with: 'refund',
        ...params,
      }),
    get: (tracking_number: string) =>
      HttpClient.get<Order>(`${API_ENDPOINTS.ORDERS}/tracking/${tracking_number}`),
  ...
  users = {
    ...
    updateContact: (input: SendOtpCodeInputType) =>
    ...
    verifyEmail: (input: any) => {
      return HttpClient.post<any>(API_ENDPOINTS.VERIFY_EMAIL, input)
    },
